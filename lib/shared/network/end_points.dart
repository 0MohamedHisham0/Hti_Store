

const CREATE_USER = 'auth/register';
const UPDATE_ROLE = 'auth/edit-user-roles';
const LOGIN = 'auth/login/user';
const UPDATE_PASSWORD = 'auth/edit-user-password/';
const DELETE_USER = 'auth/delete-user-account/';
const GET_USER_BY_ID = 'auth/users';
const GET_ALL_USERS = 'auth/system-users';
const ADD_PRODUCT = 'product/create';
const GET_ALL_OUT_OF_STORE = 'product/get-all-outofstore';
const GET_PRODUCT_BY_ID = 'product/getByid';

const UPDATE_PRODUCT_BY_ID = 'product/update';
const GET_ALL_IN_STORE = 'store/hti-store';
const ACCEPT_PRODUCT_TO_STORE = 'store/add-product-to-store';
const GET_ORDERS = 'order/getOrders';
const UPDATE_OREDER_STATE = 'order/update';
const GET_ORDER_BY_ID = 'order/getorder';
const CREATE_ORDER = 'order/create';

